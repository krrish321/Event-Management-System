...
export default function UpdateEventPage() {
    ...
    const fetchEvent = async () => {
      try {
        ...
        if (data.id) {
          ...
          setFormData({
            location: data.location || "",
            eventDate: data.start_datetime ? format(new Date(data.start_datetime), "yyyy-MM-dd") : format(new Date(), "yyyy-MM-dd"),
            attendeesCount: data.attendees_count?.toString() || "0",
            updatingDate: format(new Date(), "yyyy-MM-dd"),
          });
        } else {
  ...
  const handleSave = async () => {
    ...
    // Prevent null/empty values for required fields
    const body = {
      eventName: event.name || "Default Event Name",
      description: event.description || "No Description",
      startDate: formData.eventDate || format(new Date(), "yyyy-MM-dd"),
      startTime: "10:00:00",
      endDate: formData.eventDate || format(new Date(), "yyyy-MM-dd"),
      endTime: "12:00:00",
      issueDate: event.issue_date || format(new Date(), "yyyy-MM-dd"),
      location: formData.location || "Default Location",
      level: event.level || "Jila",
      eventType: event.event_type || "Meeting",
      assignedUser: event.created_by || 1,
    };
