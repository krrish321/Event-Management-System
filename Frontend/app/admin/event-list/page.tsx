...
export default function AdminEventListPage() {
  ...
  return (
    <div className="container mx-auto p-4 md:p-8">
      <h1 className="text-3xl md:text-4xl font-bold mb-6 border-b-2 pb-2">Admin Event List</h1>
      {events.length === 0 ? (
        <p className="text-center text-gray-500">No events found.</p>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {events.map((event) => (
            <div key={event._id} className="bg-white rounded-lg shadow-lg p-6 hover:shadow-xl transition-shadow duration-300">
              ...
              <div className="text-sm text-gray-700 space-y-1">
                ...
                <p><strong>Date:</strong> {event.startDate ? format(new Date(event.startDate), "PPP") : "N/A"}</p>
                <p><strong>Assigned To:</strong> {event.assignedUser || "N/A"}</p>
              </div>
            </div>
          ))}
        </div>
